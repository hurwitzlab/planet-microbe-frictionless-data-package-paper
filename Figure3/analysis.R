...
temperature <- data %>% 
  ggplot(aes(Lat_bin, Temperature)) + 
  theme_linedraw() +
  geom_boxplot() +
  geom_jitter(size=1.2, alpha=3/4,width=0, aes(color=factor(Project.Name)), show.legend = FALSE ) +
  custom_colors +
  facet_grid(. ~ Environmental.Feature , scales = "free", space = "free")
plot(temperature)

path_string = paste(directory,"temperature.pdf",sep="/")

#save plot
ggsave(filename = path_string, 
       plot =  temperature, width = 29.7, height = 10, units = "cm")

oxygen <- data %>% 
  ggplot(aes(Lat_bin, Oxygen)) + 
  theme_linedraw() +
  geom_boxplot() +
  geom_jitter(size=1.2, alpha=3/4,width=0, aes(color=factor(Project.Name)), show.legend = FALSE ) +
  custom_colors +
...
path_string = paste(directory,"oxygen.pdf",sep="/")

#save plot
ggsave(filename = path_string, 
       plot =  oxygen, width = 29.7, height = 10, units = "cm")

nitrate <- data %>% 
  ggplot(aes(Lat_bin, Nitrate)) + 
  theme_linedraw() +
  geom_boxplot() +
  geom_jitter(size=1.2, alpha=3/4,width=0, aes(color=factor(Project.Name)), show.legend = FALSE ) +
  custom_colors +
...
path_string = paste(directory,"nitrate.pdf",sep="/")

#save plot
ggsave(filename = path_string, 
       plot =  nitrate, width = 29.7, height = 10, units = "cm")
